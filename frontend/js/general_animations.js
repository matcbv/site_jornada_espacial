...
if (currentMain.classList.contains('index-main')) {
	...
	const introSectionRect = document
	...
	playBtn.addEventListener('click', () => {
		...
		const redbonesSoundWaveIcon = document
			.querySelector('#redbone')
			.querySelector('.sound-wave-icon');
		if (currentAudio) {
...
function musicsFetch(music) {
	fetch('/loggedIn')
		...
		.then((userSession) => {
			if (userSession) {
				if (
					!userSession.badges.some((badgeList) =>
						badgeList.includes('musical_travaller'),
					)
				) {
							...
							if (playedMusics.length === 5) {
								setTimeout(() => {
									fetch('/getModal')
										.then((data) => data.text())
										.then((html) => {
											addModal(currentMain, html, 'musical_travaller');
										})
										.catch(() => {});
								}, 500);
							}
...
window.addEventListener('click', (e) => {
	...
	if (
		!playlistDiv.contains(element) &&
		element !== musicIcon &&
		window.getComputedStyle(playlistDiv).display === 'block'
	) {
		playlistDiv.classList.add('disappear-animation');
		setTimeout(() => {
			playlistDiv.style.display = 'none';
...
if (audioDiv) {
	if (
		!currentMain.classList.contains('ideaform-main') &&
		!currentMain.classList.contains('login-main') &&
		!currentMain.classList.contains('password-main')
	) {
		const footerRectTop = document
