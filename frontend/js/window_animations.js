...
if (introductionSection && headerSection) {
	...
	currentMain.classList.contains('galaxies-main') ||
	currentMain.classList.contains('profile-main') ||
	currentMain.classList.contains('edit-profile-main')
) {
	document.body.classList.add('change-scrollbar');
}
				...
				if (userSession) {
					if (
						!userSession.badges.some((badgeList) =>
							badgeList.includes('thinker_backpacker'),
						)
					) {
